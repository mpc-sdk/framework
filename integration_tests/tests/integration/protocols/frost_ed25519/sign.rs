...
pub(super) async fn sign_t_2(
    ...
    // Verifying key of the public key package
    let vkey = first_share.1.verifying_key().clone();
    ...
    // For each signature generated by each participant check that
    // the threshold signature can be verified by the group public
